...
class HpsGiftTest < Test::Unit::TestCase
  def setup
    @gateway = HpsGiftGateway.new(secret_api_key: '12')
  ...
  def test_successful_balance
    @gateway.expects(:ssl_post).returns(successful_purchase_response)

    response = @gateway.balance(@gift_card)
  ...
  def successful_purchase_response
    %(
      ...
      $ DEBUG_ACTIVE_MERCHANT=true ruby -Itest \
        test/remote/gateways/remote_hps_gift_test.rb \
